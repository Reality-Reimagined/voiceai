...
interface CloneVoiceResponse {
  message: string;
  output_file: string; // This should match the actual response structure
}
...
export function VoiceClonePage() {
  ...
  const handleCloneVoice = useCallback(async () => {
    ...
    try {
      ...
      setResponseFileName(response.output_file);
      ...
      const audioUrl = await downloadAudioFile(response.output_file);
  ...
  }, [audioBlob, customText, toast, clonedAudioUrl]);
  ...
  const downloadAudioFile = async (fileName: string): Promise<string> => {
    const response = await fetch(`/path/to/audio/files/${fileName}`); // Adjust the path as necessary
    if (!response.ok) {
      throw new Error('Failed to download audio file');
    }
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    return url; // Return the URL for the audio file
  };
...
// // this one is bad dont revert deploys but weird cache
// import { useState, useCallback } from 'react';
// import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
// import { useToast } from '@/components/ui/use-toast';
// import { AudioRecorder } from '@/components/voice-clone/AudioRecorder';
