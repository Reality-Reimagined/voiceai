...
export const cloneVoice = async (audioFile: File, refText: string, customText: string) => {
  try {
    ...
    const formData = new FormData();
    // Make sure the file is sent with the correct field name
    formData.append('audio_file', audioFile);
    // Add the text fields with Form data
    formData.append('text', customText);
    formData.append('ref_text', refText);

    const response = await fetch(`${API_CONFIG.TTS_SERVICE_URL}/voiceClone/`, {
      method: 'POST',
      body: formData,
    });
    ...
    if (!audioResponse.ok) {
      throw new Error('Failed to fetch the generated audio');
    }
...
// old ones that worked by switching - no temp audio
// export const cloneVoice = async (audioFile: File, refText: string, customText: string) => {
//   try {
//     // Validate file type
